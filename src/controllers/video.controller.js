...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    // sortBy can be view ,createdAt ,duration
    ...
    const videoAggregate = Video.aggreagate(pipeline);
...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    const videoLocalPath = req.files?.path[0];
    const thumbnailLocalPath = req.files?.path[0];
...
const getVideoById = asyncHandler(async (req, res) => {
    ...
    if (!video) {
        throw new apiError(400, "failed to fetched video ");
    }
...
// toggle publish and unpublish video
...
const togglePublishStatus = asyncHandler(async (req, res) => {
    ...
    if (video?.owner.toString() !== req.user?._id.toString()) {
        throw new apiError(400, "You can't update this video as you are not the owner");
    }
    ...
    if (!toggleVideoPublish) {
        throw new apiError(500, "Failed to update video publish status please try again");
    }

    return res.status(200).json(
        200, { isPublished: toggleVideoPublish.isPublished }, "Video publish status updated successfully"
    )
